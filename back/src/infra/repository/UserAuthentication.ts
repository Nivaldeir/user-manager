...
export default class UserAuthenticationDatabase
  ...
  constructor(private readonly db: Prisma.UserAuthenticationDelegate<DefaultArgs>) {
  ...
  async updateForRoleUser({ userId, role }: { userId: string, role: string }) {
    try {
      console.log(role)
      return await this.db.update({
        where: {
  ...
  async find(input?: any): Promise<UserAuthentication[]> {
    try {
      ...
      let userPermissions = output.map(user => {
        let permissions = [...user.permission.map(permission => permission.name), ...user.role.permission.map(permission => permission.name)]
        return {
          ...user,
          role: user.role.name,
