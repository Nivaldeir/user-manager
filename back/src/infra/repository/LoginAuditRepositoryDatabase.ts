...
export default class LoginAuditRepositoryDatabase implements ILoginAuditRepository {
  ...
  async save(input: Omit<LoginAudit, "id">): Promise<any> {
    const output = await this.db.create({
      data: {
        device: input.device,
        location: input.location ?? "Não localizado",
        ip: input.ip,
  ...
  async update(input: Partial<Omit<LoginAudit, "id">> & { id: string; }): Promise<any> {
