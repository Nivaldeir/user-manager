...
export class UsecaseFactory {
  ...
  static create(): UsecaseFactory {
    ...
    const userAuthRepository = new UserAuthenticationDatabase(db.userAuthentication);
    const loginAuditRepository = new LoginAuditRepositoryDatabase(db.loginAudit);
    const permissionRepository = new PermissionRepositoryDatabase(db.permission);
    ...
    const observadorDeLogin = new ObservableAuthentication(loginAuditRepository, emailRepository,userService);
    userService.add(observadorDeLogin)
    return new UsecaseFactory(
      new AuthService(userAuthRepository, tokenService, loginAuditRepository, passwordService, emailRepository).add(observadorDeLogin),
