...
export interface IUserAuthenticationRepository
  ...
  deleteByPermission({ userId, permission }: { userId: string, permission: string }): Promise<any>
  addingByPermission({ userId, permission }: { userId: string, permission: string }): Promise<any>
  updateForRoleUser({ userId, role }: { userId: string, role: string }): Promise<any>
}
